...
namespace Xamarin.Forms.CommonCore
{
	...
	{
		Task<GenericResponse<List<T>>> GetAll<T>(bool includeDeleted = false) where T : ISqlDataModel, new();
		Task<BooleanResponse> TruncateAsync<T>() where T : ISqlDataModel, new();
		Task<GenericResponse<T>> GetByInternalId<T>(Guid CorrelationID, bool includeDeleted = false) where T : class, ISqlDataModel, new();
		...
		Task<BooleanResponse> DeleteByInternalID<T>(Guid ID, bool softDelete = true) where T : class, ISqlDataModel, new();
		Task<BooleanResponse> DeleteByQuery<T>(Expression<Func<T, bool>> exp, bool softDelete = true) where T : ISqlDataModel, new();
	}
}
